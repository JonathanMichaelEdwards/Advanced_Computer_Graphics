...
glm::vec3 eyePos(0.0, 20.0, 30.0);  /* scene view */
...
void loadTextures()
{
	...
	loadTGA("./src/height_maps/Terrain_hm_01.tga");
...
void display()
{
	...
	*/
	view = glm::lookAt(eyePos, glm::vec3(0.0, 0.0, -40.0), glm::vec3(0.0, 1.0, 0.0)); //view matrix
	projView = proj * view;  // Product matrix

	glUniformMatrix4fv(mvpMatrixLoc, 1, GL_FALSE, &projView[0][0]);
	glUniform3fv(eyeLoc, 1, &eyePos[0]); // map value inside shader

	glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
	glBindVertexArray(vaoID);
	glDrawElements(GL_PATCHES, 81*4, GL_UNSIGNED_SHORT, NULL); // Changed GL_QUADS to GL_PATCHES
	glFlush();
}
