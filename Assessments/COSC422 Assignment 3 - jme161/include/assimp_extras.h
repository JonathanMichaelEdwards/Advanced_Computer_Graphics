...
//
// This file contains a set of helper functions commonly used in Assimp applications
//-----------------------------------------------------------------------------------------
...
static void get_bounding_box_for_node (const aiScene* scene, aiNode* nd, aiVector3D* min,  aiVector3D* max, 
	aiMatrix4x4 trafo)
{
...
// ----------------------------------------------------------------------------
static void printTreeInfo(const aiNode* node)
{
...
// ----------------------------------------------------------------------------
static void printBoneInfo(const aiScene* scene)
{
		float* mat = new float[16];
		cout << "==================== Bone Data ===========================" << endl;
		...
		for (int n = 0; n < scene->mNumMeshes; ++n)
		{
			...
			if(mesh->HasBones())
			{
				for(int i = 0; i < mesh->mNumBones; i++)
				{
					...
					cout << "      Vertex weights: " << (bone->mWeights[0]).mWeight << "  " 
						<< (bone->mWeights[bone->mNumWeights-1]).mWeight << endl;
				}
			}
		}
}
...
// ----------------------------------------------------------------------------
static void printAnimInfo(const aiScene* scene, int animIndx)
{
	...
	if(scene != NULL)
	{
		...
		cout << "Number of animations = " << scene->mNumAnimations << endl;
			...
			aiAnimation* anim = scene->mAnimations[animIndx];
			cout << " --- Anim " << animIndx << ":  Name = " << (anim->mName).C_Str() << 
				"  nchanls = " << anim->mNumChannels << " nticks per sec = " << anim->mTicksPerSecond <<
